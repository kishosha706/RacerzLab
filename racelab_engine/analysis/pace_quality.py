...
def score_platform_safety(platform_risk_peak: float | None = None, rear_platform_risk_peak: float | None = None, whole_car_bottoming_peak: float | None = None) -> float:
    scores: list[float] = []
...
def score_tire_safety(temp_spread: float | None = None, pressure_gain: float | None = None, wear_spread: float | None = None, camber_bias: float | None = None) -> float:
    scores: list[float] = []
...
def compute_pace_quality_score(
    ...
    pace_quality = clamp_score(0.35 * pace_speed + 0.25 * consistency + 0.20 * falloff + 0.08 * platform + 0.07 * tire + 0.05 * shock)
